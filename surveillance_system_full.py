...
class GlassEdgeMeasurement:
    ...
    def set_calibration(self, pixel_to_mm_ratio):
        ...
        logging.info(f"Cam kenarı kalibrasyonu ayarlandı: 1 pixel = {pixel_to_mm_ratio} mm")
    ...
    def measure_dimensions(self, glass_edges):
        ...
        measurements['edge_quality'] = self.evaluate_edge_quality(main_edge['contour'])
        
        # Defekt tespiti
        measurements['defects'] = self.detect_defects(main_edge['contour'])
        
        return measurements
        
    def evaluate_edge_quality(self, contour):
        """Kenar kalitesini değerlendirir (0-100 arası skor)"""
        if contour is None or len(contour) < 10:
            return 0
            
        # Konturun düzgünlüğünü kontrol et
        ...
        # Düzgünlük skoru (0-100)
        quality_score = min(100, convexity * 100)
        
        return quality_score
        
    def detect_defects(self, contour):
        """Cam kenarındaki defektleri tespit eder"""
        defects_list = []
        ...
        if len(hull) > 3:
            ...
            if defects is not None:
                for i in range(defects.shape[0]):
                    ...
                    # Defekt derinliği (pixel)
                    defect_depth = d / 256.0
                    
                    # Defekt derinliği eşiğini aş
                    if defect_depth > 5:  # 5 pixel
                        defects_list.append({
                            'type': 'concavity',
                            'position': far,
                            'depth': defect_depth * self.pixel_to_mm_ratio,
                            'severity': 'high' if defect_depth > 15 else 'medium'
                        })
    ...
    def save_measurement(self, measurements):
        ...
        # CSV'ye kaydet
        self.save_to_csv(measurement_record)
        
    def save_to_csv(self, record):
        """Ölçüm sonuçlarını CSV dosyasına kaydet"""
        csv_file = 'glass_measurements.csv'
        file_exists = os.path.isfile(csv_file)
        
        with open(csv_file, 'a', newline='', encoding='utf-8') as file:
            fieldnames = ['timestamp', 'width', 'height', 'perimeter', 'area', 'edge_quality', 'within_tolerance']
            ...
            writer.writerow({
                ...
                'edge_quality': measurements.get('edge_quality', 0),
...
class GlassEdgeSettingsDialog(QDialog):
    ...
    def setup_ui(self):
        ...
        # Ölçüm etkinleştirme
        self.enable_measurement_cb = QCheckBox("Cam Kenarı Ölçümünü Etkinleştir")
        general_layout.addWidget(self.enable_measurement_cb)
        ...
        filter_layout.addWidget(QLabel("Morphology Kernel:"), 1, 0)
        self.morph_kernel_spin = QSpinBox()
        self.morph_kernel_spin.setRange(0, 15)
        filter_layout.addWidget(self.morph_kernel_spin, 1, 1)
        
        filter_layout.addWidget(QLabel("Min Kenar Uzunluğu:"), 2, 0)
        self.min_edge_length_spin = QSpinBox()
        self.min_edge_length_spin.setRange(10, 1000)
        filter_layout.addWidget(self.min_edge_length_spin, 2, 1)
        ...
        calib_grid.addWidget(QLabel("Ölçüm Birimi:"), 0, 0)
        self.unit_combo = QComboBox()
        self.unit_combo.addItems(["mm", "cm", "pixel"])
        calib_grid.addWidget(self.unit_combo, 0, 1)
        
        calib_grid.addWidget(QLabel("Pixel/mm Oranı:"), 1, 0)
        ...
        calib_grid.addWidget(self.pixel_mm_ratio_spin, 1, 1)
        
        calib_layout.addWidget(calib_group)
        
        # Tolerans ayarları
        tolerance_group = QGroupBox("Tolerans Ayarları")
        tolerance_layout = QGridLayout(tolerance_group)
        
        tolerance_layout.addWidget(QLabel("Tolerans Aralığı:"), 0, 0)
        self.tolerance_spin = QDoubleSpinBox()
        self.tolerance_spin.setRange(0.1, 50.0)
        self.tolerance_spin.setDecimals(2)
        self.tolerance_spin.setSuffix(" mm")
        tolerance_layout.addWidget(self.tolerance_spin, 0, 1)
        
        calib_layout.addWidget(tolerance_group)
    ...
    def load_current_settings(self):
        ...
        self.enable_measurement_cb.setChecked(gm.enabled)
        ...
        self.morph_kernel_spin.setValue(gm.morphology_kernel_size)
        self.min_edge_length_spin.setValue(gm.min_edge_length)
        self.unit_combo.setCurrentText(gm.measurement_unit)
    ...
    def apply_settings(self):
        ...
        gm.enabled = self.enable_measurement_cb.isChecked()
        ...
        gm.morphology_kernel_size = self.morph_kernel_spin.value()
        gm.min_edge_length = self.min_edge_length_spin.value()
        gm.measurement_unit = self.unit_combo.currentText()
...
class VideoDisplayWidget(QLabel):
    ...
    def draw_glass_measurements(self, painter):
        ...
        if 'bounding_rect' in measurements:
            ...
            info_text.append(f"Kenar Kalitesi: {measurements.get('edge_quality', 0):.1f}%")
            
            # Defektleri göster
            defects = measurements.get('defects', [])
            if defects:
                info_text.append(f"Defekt Sayısı: {len(defects)}")
        ...
        for defect in defects:
            pos = defect['position']
            painter.setPen(QPen(QColor(255, 0, 0), 3))  # Kırmızı
            painter.drawEllipse(QPoint(pos[0], pos[1]), 5, 5)
...
class SurveillanceSystem(QMainWindow):
    ...
    def update_glass_info_display(self, measurements, tolerance_results):
        ...
        info_text += f"Kenar Kalitesi: {measurements.get('edge_quality', 0):.1f}%\n"
        
        defects = measurements.get('defects', [])
        info_text += f"Defekt Sayısı: {len(defects)}\n"
